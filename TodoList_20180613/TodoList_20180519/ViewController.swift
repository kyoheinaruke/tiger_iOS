...
//        tableView.addGestureRecognizer(longPressGesture)
    ...
    @IBAction func tapAddButton(_ sender: Any) {
        //アラートダイアログを作成
        let alertAdd = UIAlertController(title: "ToDo追加" , message: "ToDoを入力してください", preferredStyle: UIAlertControllerStyle.alert)
        
        //テキストエリアを追加
        alertAdd.addTextField(configurationHandler: nil)
        
        //追加ボタンを追加
        let okAction = UIAlertAction(title: "追加", style: UIAlertActionStyle.default) { (action: UIAlertAction) in
            //OKボタンがタップされた時の処理
            if let textField = alertAdd.textFields?.first {
        ...
        alertAdd.addAction(cancelButton)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //選択したセルの情報を取得
        let myTodo = todoList[indexPath.row]
        let todoText = myTodo.todoTitle
        
        //アラートダイアログを作成
        let alertEdit = UIAlertController(title: "ToDo編集" , message: "内容を編集してください", preferredStyle: UIAlertControllerStyle.alert)
        
        //テキストエリアを追加し、選択したテキストをデフォルト値とする
        alertEdit.addTextField(configurationHandler: {(textField: UITextField!) -> Void in
            textField.text = todoText
        })
        
        //追加ボタンを追加
        let okAction = UIAlertAction(title: "完了", style: UIAlertActionStyle.default) { (action: UIAlertAction) in
            //OKボタンがタップされた時の処理
            
            if let textField = alertEdit.textFields?.first {
                //ToDoの配列に入力値を挿入。先頭に挿入する。
                myTodo.todoTitle = textField.text!
...
//    func cellLongPressed(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath,recognizer: UILongPressGestureRecognizer) {
//        //選択したセルの情報を取得
//        let myTodo = todoList[indexPath.row]
//
//        if myTodo.todoDone{
...
//        let todoCell = sender.view!
//
//        if(sender.state == UIGestureRecognizerState.began) {
//
//        } else if (sender.state == UIGestureRecognizerState.ended) {
//            let indexPath: IndexPath = tableView.indexPath(for: todoCell.superview as! UITableViewCell)!
//
//            let myTodo = todoList[indexPath.row]
//
//            if myTodo.todoDone{
