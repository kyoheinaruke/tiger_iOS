...
class SearchItemTableViewController: UITableViewController, UISearchBarDelegate{
    ...
    override func viewWillAppear(_ animated: Bool) {
        //一度全て削除してから保存しているデータを入れる（画面遷移があるため）
    ...
    func request(requestUrl: String) {
        ...
        let task = session.dataTask(with: request) { (data:Data?,
            ...
            do {
                ...
                let alert = UIAlertController(title: "検索結果0件",
                                              message: "検索キーワードを変更してください",
                                              preferredStyle: UIAlertControllerStyle.alert)
                //OKボタンを追加
                let OKButton = UIAlertAction(title: "OK",style: UIAlertActionStyle.cancel, handler: nil)
                //OKボタンがタップされたときの処理
    ...
    @objc func longPress(sender: UILongPressGestureRecognizer){
        ...
        if sender.state == UIGestureRecognizerState.began{
            ...
            let favoriteList = MyItemList()
            
            favoriteList.name = itemList.name
            favoriteList.url = itemList.url
            favoriteList.medium = itemList.imageInfo.medium
            favoriteList.price = itemList.priceInfo.price
            favoriteList.review = itemList.reviewInfo.review
            favoriteList.count = itemList.reviewInfo.count
            
            self.favoriteList.insert(favoriteList, at: 0)
            
            
            //お気に入り情報の保存処理
            let userDefaults = UserDefaults.standard
            // Date型にシリアライズする
            let date = NSKeyedArchiver.archivedData(withRootObject: self.favoriteList)
            userDefaults.set(date, forKey: "favoriteList")
            userDefaults.synchronize()
            
            //追加されたことを表示
            let alert = UIAlertController(title: "お気に入りに追加されました",
                                          message: "「履歴/気になる」から確認できます",
                                          preferredStyle: UIAlertControllerStyle.alert)
            //OKボタンを追加
            let OKButton = UIAlertAction(title: "OK",style: UIAlertActionStyle.cancel, handler: nil)
            //OKボタンがタップされたときの処理
